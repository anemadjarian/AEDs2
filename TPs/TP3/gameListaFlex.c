...
void inserirInicio(Lista* l, Game* g) {
    ...
    l->primeiro = nova;
    if (l->ultimo == NULL) l->ultimo = nova;
    l->tamanho++;
}
...
void inserirFim(Lista* l, Game* g) {
    ...
    if (l->ultimo) {
        ...
        l->primeiro = l->ultimo = nova;
    }

    l->tamanho++;
}

void inserirPosicao(Lista* l, Game* g, int pos) {
...
Game* removerPosicao(Lista* l, int pos) {
    if (pos < 0 || pos >= l->tamanho) {
        fprintf(stderr, "Posição inválida: %d\n", pos);
        return NULL;
    }
...
Game* removerFim(Lista* l) {
    if (l->primeiro == NULL) return NULL; // lista vazia
    ...
    // mais de um: percorre até o penúltimo
    Celula* p = l->primeiro;
    while (p->prox != l->ultimo) p = p->prox;
...
int main(void) {
    ...
    for (int i = 0; i < n; i++) {
        char linhaCmd[100];
        fgets(linhaCmd, sizeof(linhaCmd), stdin);
        linhaCmd[strcspn(linhaCmd, "\r\n")] = '\0';

        char cmd[5];
        int pos, id;

        if (sscanf(linhaCmd, "%s %d %d", cmd, &pos, &id) >= 1) {

            if (strcmp(cmd, "II") == 0) {
                Game* g = buscarPorId(jogos, total, pos); // 'pos' contém o id neste caso
                if (g) inserirInicio(&lista, g);

            } else if (strcmp(cmd, "IF") == 0) {
                Game* g = buscarPorId(jogos, total, pos);
                if (g) inserirFim(&lista, g);

            } else if (strcmp(cmd, "I*") == 0) {
                Game* g = buscarPorId(jogos, total, id);
                if (g) inserirPosicao(&lista, g, pos);
                ...
                Game* r = removerPosicao(&lista, pos);
                if (r) printf("(R) %s\n", r->name);
