...
public class GameLista {
    ...
    private static List<String> parseList(String s) {
        ...
        Set<String> set = new LinkedHashSet<>(); // Cria a lista final
        for (String p : parts) {
            String item = clean(p);
            if (!item.isEmpty())
                set.add(item); // Verifica se não é vazia e não está duplicado e adiciona na lista final
        }
    ...
    public static Map<Integer, GameLista> loadCSV(String file) throws IOException {
        // Mapa= id e valor(objeto GameLista)
        Map<Integer, GameLista> map = new HashMap<>(); // Mapa vazio
    ...
    static int parseIntLista(String comando) {
        String x = "";
        if (comando.charAt(1) == 'I' || comando.charAt(1) == 'F' || comando.charAt(1) == '*')
            x = comando.substring(3);
        return Integer.parseInt(x.trim());
    }

    static int parsePosicao(String comando) {
        return Integer.parseInt(comando.substring(3, 5).trim());
    }

    static void inserirInicio(List<GameLista> lista, Map<Integer, GameLista> allGames, int id) {
        if (allGames.containsKey(id))
            lista.add(0, allGames.get(id));
    }

    static void inserirFim(List<GameLista> lista, Map<Integer, GameLista> allGames, int id) {
        if (allGames.containsKey(id))
            lista.add(allGames.get(id));
    }

    static void inserir(List<GameLista> lista, Map<Integer, GameLista> allGames, int pos, int id) {
        if (allGames.containsKey(id) && pos >= 0 && pos <= lista.size())
            lista.add(pos, allGames.get(id));
    }

    static void removerInicio(List<GameLista> lista) {
        if (!lista.isEmpty()) {
            System.out.println("(R) " + lista.remove(0).name);
        }
    }

    static void removerFim(List<GameLista> lista) {
        if (!lista.isEmpty()) {
            System.out.println("(R) " + lista.remove(lista.size() - 1).name);
        }
    }

    static void remover(List<GameLista> lista, int pos) {
        if (pos >= 0 && pos < lista.size()) {
            System.out.println("(R) " + lista.remove(pos).name);
        }
    }
    ...
    public static void main(String[] args) throws Exception {
        Map<Integer, GameLista> allGames = loadCSV("/tmp/games.csv");
        Scanner sc = new Scanner(System.in);
        List<GameLista> lista = new ArrayList<>();
        ...
        for (int i = 0; i < numComandos; i++) {
            ...
            if (comando.charAt(0) == 'I') {
                if (comando.charAt(1) == 'I') {
                    ...
                    inserirInicio(lista, allGames, id);
                } else if (comando.charAt(1) == 'F') {
                    int id = parseIntLista(comando);
                    inserirFim(lista, allGames, id);
                } else if (comando.charAt(1) == '*') {
                    int pos = Integer.parseInt(comando.substring(3, 5).trim());
                    int id = Integer.parseInt(comando.substring(6).trim());
                    inserir(lista, allGames, pos, id);
                }
                ...
                if (comando.charAt(1) == 'I') {
                    ...
                    int pos = Integer.parseInt(comando.substring(3).trim());
                    remover(lista, pos);
                }
            }
        }
